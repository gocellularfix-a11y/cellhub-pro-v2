...
export class CompanionLiteApiError extends Error {
  constructor(
    ...
    this.name = 'CompanionLiteApiError';
  }
}
...
async function send<T>(
  ...
  try {
    ...
    if (name === 'AbortError') {
      throw new CompanionLiteApiError(0, 'timeout', `Timed out after ${timeoutMs}ms`);
    }
    throw new CompanionLiteApiError(0, 'network_error', err instanceof Error ? err.message : String(err));
  }
  clearTimeout(timer);
  ...
  if (!res.ok) {
    ...
    throw new CompanionLiteApiError(res.status, reason);
  }
  return data as T;
}
