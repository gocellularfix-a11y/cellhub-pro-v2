...
const PRINT_STRINGS: Record<OrgLocale, Record<string, string>> = {
  en: {
    ...
    incomeBreakdown: 'Income Breakdown', cogsDetail: 'Cost of Goods Sold (COGS)',
    source: 'Source', gross: 'Gross', net: 'Net', vendor: 'Vendor', date: 'Date', items: 'Items',
  },
  es: {
    ...
    incomeBreakdown: 'Desglose de Ingresos', cogsDetail: 'Costo de Bienes Vendidos (COGS)',
    source: 'Fuente', gross: 'Bruto', net: 'Neto', vendor: 'Proveedor', date: 'Fecha', items: 'Artículos',
  },
  pt: {
    ...
    incomeBreakdown: 'Detalhamento de Receitas', cogsDetail: 'Custo das Mercadorias (COGS)',
    source: 'Origem', gross: 'Bruto', net: 'Líquido', vendor: 'Fornecedor', date: 'Data', items: 'Itens',
  },
};
...
export function organizerToPrintHtml(org: TaxOrganizer): string {
  ...
  // R-TAX-ORGANIZER-PDF-FULL-PACKAGE-V1: granular income + COGS detail. These
  // live in the org model (org.income / org.cogs) but were previously omitted
  // from the printable package — the PDF/print organizer now includes them so it
  // is the COMPLETE accountant package, not just the summary. Display-only:
  // every value is an already-computed string from buildTaxOrganizer (no tax
  // math is performed or duplicated here).
  const inc = org.income;
  const incomeRows = [
    `<tr><td>POS — Product / Service</td><td class="r">$${escHtml(inc.pos.productGross)}</td><td class="r">$${escHtml(inc.pos.productCOGS)}</td><td class="r">$${escHtml(inc.pos.productProfit)}</td></tr>`,
    `<tr><td>POS — Phone Commission</td><td class="r">$${escHtml(inc.pos.phoneGross)}</td><td class="r">$${escHtml(inc.pos.phonePaidToCarrier)}</td><td class="r">$${escHtml(inc.pos.phoneNetCommission)}</td></tr>`,
    `<tr><td>POS — Repair</td><td class="r">$${escHtml(inc.pos.repairRevenue)}</td><td class="r">$${escHtml(inc.pos.repairCOGS)}</td><td class="r">$${escHtml(inc.pos.repairProfit)}</td></tr>`,
    ...inc.manualEntries.map((m) =>
      `<tr><td>Manual — ${escHtml(m.category)}${m.source ? ' · ' + escHtml(m.source) : ''}</td><td class="r">—</td><td class="r">—</td><td class="r">$${escHtml(m.amount)}</td></tr>`),
    `<tr><td>Adjustment — Other Income</td><td class="r">—</td><td class="r">—</td><td class="r">$${escHtml(inc.adjustments.otherIncome)}</td></tr>`,
    `<tr><td>Adjustment — Returns / Refunds</td><td class="r">—</td><td class="r">—</td><td class="r">$${escHtml(inc.adjustments.returnsRefunds)}</td></tr>`,
  ].join('');
  const cg = org.cogs;
  const cogsSupplierRows = cg.supplierRows.map((s) =>
    `<tr><td>${escHtml(s.date)}</td><td>${escHtml(s.vendor)}</td><td>${escHtml(s.items)}</td><td class="r">$${escHtml(s.amount)}</td></tr>`).join('');
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escHtml(S.title)} ${org.meta.year}</title><style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    <!-- ── Page 2: income breakdown · COGS detail (full accountant package) ── -->
    <div class="page">
      <div class="section">
        <h2>${escHtml(S.incomeBreakdown)}</h2>
        <table>
          <thead><tr><th>${escHtml(S.source)}</th><th class="r">${escHtml(S.gross)}</th><th class="r">${escHtml(S.cogs)}</th><th class="r">${escHtml(S.net)}</th></tr></thead>
          <tbody>${incomeRows}</tbody>
          <tfoot><tr><th>${escHtml(S.totalIncome)}</th><th></th><th></th><th class="r">$${escHtml(inc.total)}</th></tr></tfoot>
        </table>
      </div>

      <div class="section">
        <h2>${escHtml(S.cogsDetail)}</h2>
        <table><tbody>
          <tr><td>Beginning Inventory</td><td class="r">$${escHtml(cg.beginningInventory)}</td></tr>
          <tr><td>Supplier Purchases</td><td class="r">$${escHtml(cg.supplierPurchases)}</td></tr>
          <tr><td>COGS Category Expenses</td><td class="r">$${escHtml(cg.cogsCategoryExpenses)}</td></tr>
          <tr><td>Refunded Returns</td><td class="r">$${escHtml(cg.refundedReturns)}</td></tr>
          <tr><td>Ending Inventory</td><td class="r">$${escHtml(cg.endingInventory)}</td></tr>
          <tr><th>${escHtml(S.cogs)}</th><th class="r">$${escHtml(cg.total)}</th></tr>
        </tbody></table>
        ${cogsSupplierRows ? `<table style="margin-top:6px"><thead><tr><th>${escHtml(S.date)}</th><th>${escHtml(S.vendor)}</th><th>${escHtml(S.items)}</th><th class="r">${escHtml(S.amount)}</th></tr></thead><tbody>${cogsSupplierRows}</tbody></table>` : ''}
      </div>

      ${pageLabel(2, 3)}
    </div>

    <!-- ── Page 3: member split · warnings & notes · preparer notes ── -->
    <div class="page">
