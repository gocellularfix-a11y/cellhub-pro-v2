// ============================================================
// Companion — Pending-notifications store.
//
// In-memory only (no persistence — clears on reload). Tracks how many
// unread Companion events the operator has and which sub-tab the
...
//   - navigateToCompanion()— captures source, resets count,
//                                stages a routeHint for CompanionPage
//   - consumeRouteHint()       — CompanionPage calls this to read
...
export interface PendingState {
  /** How many Companion items are unattended. */
  count: number;
  /** The sub-tab the latest event came from. Drives the badge click route. */
  lastSource: PendingSource | null;
  /** Staged sub-tab for CompanionPage to consume on its next render. */
 ...
 *  routeHint so CompanionPage opens that sub-tab, and zeros the
 *  count. Returns the captured source for the caller's convenience. */
export function navigateToCompanion(): PendingSource | null {
...
/** Called by CompanionPage to read + clear the staged sub-tab. */
