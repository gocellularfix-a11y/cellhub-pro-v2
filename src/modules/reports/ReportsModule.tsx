...
// CELLHUB-INTELLIGENCE-I1: the ENTIRE money calculation (inclusion rules,
// void/refund/return treatment, tax buckets, fees, COGS, commission,
// category/provider/carrier totals, Z-tape recon) moved VERBATIM to the
// canonical service. This module now only renders what the service returns —
// it no longer owns any parallel money math.
import {
  computeReportMoney,
  toDateSafe,
...
// R-DASHBOARD-PROFIT-RECONCILE-V1 compatibility: Dashboard.tsx imports these
// from this module — re-export from the canonical service (single source).
export {
  isPseudoItem,
...
// ── Constants & helpers ──────────────────────────────────────
// (Money constants + date coercion moved to computeReportMoneyStats.ts.)
...
// (isCountableSale, isPseudoItem, proportional-cost helpers → canonical
// service; re-exported above for Dashboard compatibility.)
...
// (getUnlockProportionalCost, normalizeCategoryKey, isRepairCompleted,
// isUnlockCompleted → canonical service; imported above.)
...
// (NormalizedReturn interface → canonical service; imported above.)
...
export default function ReportsModule() {
  ...
  // ── CELLHUB-INTELLIGENCE-I1: canonical money pipeline ─────
  // Inclusion rules + core stats are computed by the canonical service
  // (verbatim extraction of the logic that lived inline here). The derived
  // collections keep their original names so every render/print/export
  // consumer below is untouched.
  const reportMoney = useMemo(() => computeReportMoney({
    sales: safeSales,
    repairs: safeRepairs,
    unlocks: safeUnlocks,
    specialOrders: safeSpecialOrders,
    layaways: safeLayaways,
    inventory,
    customerReturns: Array.isArray(customerReturns) ? customerReturns : [],
    vendorReturns: safeVendorReturns,
    settings,
    periodRange,
    labels: {
      noProvider: t('reports.noProvider'),
      noCarrier: t('reports.noCarrier'),
      unknownEmployee: t('reports.unknownEmployee'),
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // ── Returns (live AppState) — normalized by the canonical service; the
  // `allReturns` rows above come from reportMoney.collections.
  ...
  // (returnsInPeriod / returnsFromPeriodSales / repair+unlock in-sale
  // tracking / standalone derivations → canonical service; destructured
  // from reportMoney.collections above.)
  ...
  // ============================================================
  //  CORE STATS — computed by the canonical service (CELLHUB-
  //  INTELLIGENCE-I1). `stats` is destructured from reportMoney above;
  //  the single-loop cents pipeline lives VERBATIM in
  //  src/services/reports/computeReportMoneyStats.ts.
  // ============================================================
